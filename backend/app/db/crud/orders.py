...
async def create_order(session, order_in):
    order = Order(order_reference=order_in.order_reference, area_id=order_in.area_id)
    session.add(order)
    await session.flush()

    for line in order_in.lines:
        ol = OrderLine(order_id=order.id, item_id=line.item_id, quantity=line.quantity)
        session.add(ol)

        q = await session.execute(
            select(Inventory).where(
                Inventory.item_id == line.item_id,
                Inventory.area_id == order_in.area_id
            ).with_for_update()
        )
        inv = q.scalars().first()
        if inv is None:
    ...
    await session.commit()  # Commit after flushing all order lines and inventory updates
    return order
